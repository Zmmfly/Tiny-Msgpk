...
MessagePack是一个像JSON一样的对象序列化规范。

MessagePack has two concepts: **type system** and **formats**.

MessagePack有两个概念: **type system** and **formats**.

序列化是通过MessagePack type系统将应用对象转换为MessagePack formats。

反序列化是通过MessagePack type系统将MessagePack formats转换为应用对象
...
本文档介绍了 MessagePack type 系统, MessagePack formats 以及它们的转换。
...
* MessagePack specification
  ...
  * [序列化: type 到 format 转换](#serialization-type-to-format-conversion)
  * [反序列化: format 到 type 转换](#deserialization-format-to-type-conversion)
...
* 字符串对象可能包含无效字节序列(ex:\0), 反序列化器的行为取决于当接收到无效字节序列的实际实现
...
应用可以分配`0`至`127`来存储专用类型信息. 一个使用例子是应用定义`type=0`作为应用的唯一类型系统, 并将类型的名称和数值存储在载荷当中
...
MessagePack 保留 `-1`至`-128` 以供在将来扩展添加预定义类型. 将添加这些类型以交换更多类型，而无需在不同的编程环境中使用预共享的静态类型的架构。 
